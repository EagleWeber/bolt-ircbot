...
func RandomInsult() string {
	...
	return fmt.Sprintf("calls out %v, the %v %v %v!", "%v", insult1, insult2, insult3)
}
	...
	ircproj.AddCallback("PRIVMSG", func(event *irc.Event) {
		...
		if len(matches) > 0 {
			...
			for _, element := range tokens {
				...
				if element == "gawainlynch" {
					ircproj.Actionf(event.Arguments[0], "Do you think I am crazy, %v? He created me, and can delete me just as quickly!", event.Nick)
				} else {
					time.Sleep(100)
