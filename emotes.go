...
func AddPrivmsgDocs(ircproj *irc.Connection) error {
  ...
  screenshots := regexp.MustCompile(`#(screenshot|screenshots)`)
  taxonomies := regexp.MustCompile(`#(taxonomy|taxonomies)`)
  menus := regexp.MustCompile(`#(menu|menus)`)
  relationships := regexp.MustCompile(`#(relationship|relationships)`)
  templates := regexp.MustCompile(`#(template|templates)`)
  records := regexp.MustCompile(`#(record|records)`)
  paging := regexp.MustCompile(`#(pages|paging)`)
  search := regexp.MustCompile(`#(search|searching)`)
  temptags := regexp.MustCompile(`#(tags|templatetags)`)
  internals := regexp.MustCompile(`#internals`)
